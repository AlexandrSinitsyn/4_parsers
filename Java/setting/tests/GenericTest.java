...
public class GenericTest extends BaseTest {
        ...
        for (int x = x1; x <= x2; x++) {
            for (int y = y1; y <= y2; y++) {
                for (int z = z1; z <= z2; z++) {
                    ...
                    assert equals(mode, expected, actual) :
    ...
    private static boolean equals(final String mode, final Object expected, final Object actual) {
        return "d".equals(mode) ? equals((Double) expected, (Double) actual) : Objects.equals(expected, actual);
    }

    private static boolean equals(final double expected, final double actual) {
        final double delta = Math.abs(expected - actual);
        return Double.compare(expected, actual) == 0 || delta < EPS * Math.max(1, Math.abs(expected));
    }
